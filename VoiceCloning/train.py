...
from Melgan.train_melgan import MelganTrainer, collater
    ...
    def _write_to_tensorboard(self, list_metrics, stage="train"):
        ...
        with self.writer.as_default():
            for key, value in list_metrics.items():
                if isinstance(value, tf.keras.metrics):
                    tf.summary.scalar(stage + "/" + key, value.result(), step=self.steps)
                else:
                    tf.summary.histogram(stage + "/" + key, value, step=self.steps)
                self.writer.flush()
...
class MultiBandMelganTrainer(MelganTrainer):
    ...
    def generate_and_save_intermediate_result(self, batch):
        ...
        y_mb_batch_ = y_mb_batch_['y_mb_hat']
