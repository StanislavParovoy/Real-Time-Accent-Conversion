    ...
    for i in range(n_blocks):
      layers += [
        tf.keras.layers.Conv1D(filters, 
                               kernel_size=kernel_size+(i==n_blocks//2), 
                               padding='same'),
                               # strides=1+(i==n_blocks//2)),
...
'''
class VQ_EMA():
  def __init__(self, latent_dim, k, beta=parameters.beta, scope='vqvae', 
               trainable=True, decay=0.998, epsilon=1e-5):
