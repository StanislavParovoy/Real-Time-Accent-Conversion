...
sys.path.append('.')
from GE2E.model import GE2E
from GE2E.losses import SoftmaxLoss
from GE2E.dataset import Libri
import tensorflow as tf
from utils import suppress_tf_warning
...
class Trainer():
  ...
  def _train_step(self, data):
    ...
    # accuracy = tf.nn.compute_average_loss(accuracy, global_batch_size=global_batch_size)
    # eer = tf.nn.compute_average_loss(eer, global_batch_size=global_batch_size)

    variables = self.model.trainable_variables
    gradients = tf.gradients(loss, variables)
    # gradients, _ = tf.clip_by_global_norm(gradients, hparams.global_norm)
...
def main():
  ...
  with open(config['melspectrogram']) as f:
    mel_config = yaml.load(f, Loader=yaml.Loader)
  config.update(mel_config['speech_config'])
