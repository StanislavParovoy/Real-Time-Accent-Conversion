...
from conv_blocks import conv_actv, conv_bn_actv, conv_ln_actv, conv_in_actv, conv_bn_res_bn_actv
...
class TDNNEncoder(Encoder):
  ...
  def _encode(self, input_dict):
    ...
    if False and isinstance(self._model.get_data_layer(), Speech2TextDataLayer):
      pad_to = 0
    ...
    if False and self.params.get("use_conv_mask", False):
    ...
    for idx_convnet in range(len(convnet_layers)):
      ...
      for idx_layer in range(layer_repeat):
        ...
        if self.params.get("use_conv_mask", False):
          ...
          if False and idx_layer > 0 and self.params.get("use_conv_mask", False):
            conv_feats = conv_feats * mask
